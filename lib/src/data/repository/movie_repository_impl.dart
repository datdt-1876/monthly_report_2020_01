...
class MovieRepositoryImpl extends MovieRepository {
  ...
  @override
  Future<List<Movie>> fetchMoviesByQueryString(String queryString, int page) async {
    final response = await _client.get('https://api.themoviedb.org/3/search/movie?api_key=$API_KEY&language=en-US&query=$queryString&page=$page');
    if (response.statusCode == 200) {
      return MovieResponse.parserFromJson(json.decode(response.body))?.movies ?? List.empty();
    } else {
