...
class MovieBloc extends Bloc<MovieEvent, MovieState> {
  ...
  Stream<MovieState> mapEventToState(MovieEvent event) async* {
    ...
    } else if(event is FetchMoviesByQueryString) {
      try {
        final movies = await movieRepository.fetchMoviesByQueryString(event.queryString, page);
        yield MoviesByQueryFetched(movies);
        if(movies.isEmpty) isEnd = true;
        page++;
        return;
      } on Exception catch(e) {
        yield MovieFetchError(e.toString());
